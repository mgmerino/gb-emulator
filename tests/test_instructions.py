"""The instruction set: one section per block, plus the cycle rule.

Each block is tested through its generator rather than entry by entry -- a
handful of decoded opcodes, the cycle costs, and the closure trap that would
show up if every entry had captured the same index.
"""
